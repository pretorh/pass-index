...
if PASSWORD_STORE_DIR='' PASSWORD_STORE_EXTENSIONS_DIR='' PASSWORD_STORE_ENABLE_EXTENSIONS=false pass index 2>"$LOG_FILE" ; then
    uses_system_pass="test will incorrectly use system-wide index extension and fail"
fi
skip_if "$uses_system_pass" index_file_is_named_different_to_not_clash_with_implicit_show_in_pass
